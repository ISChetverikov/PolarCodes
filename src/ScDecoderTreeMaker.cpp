...
size_t GetFirstDistinctBit(size_t first, size_t second) {
	size_t x = first ^ second;
...
// Fill
void FillBinary(int number, std::vector<int> & binary, size_t lastDigitsCount) {
	size_t k = binary.size();
	for (size_t i = 0; i < lastDigitsCount; i++)
	{
		int digit = number % 2;
		number = number >> 1;
		binary[k - i - 1] = digit;
...
std::vector<int> ScDecoderTreeMaker::Decode(std::vector<double> inP1) {
	...
	size_t treePathCount = pow(2, k);

	size_t previous_number = treePathCount - 1;
	size_t current_j = 0;
	for (size_t number = 0; number < treePathCount; number++)
	{
